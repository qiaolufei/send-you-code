<template>
    <div class="postWant">
        <headpage></headpage>
        <div class="want">
            <label>需求主题</label><el-input class="input1" v-model="want.theme" placeholder="请输入需求主题"></el-input>
            <br><br>
            <label>技术标签</label>
            ...
            <br><br>
            <label>具体需求</label><el-input class="input1" type="textarea" v-model="want.content" :rows="4" placeholder="请输入具体需求内容" maxlength="500" resize="none" show-word-limit></el-input>
            <!-- <br><br>
            <label>附件</label>
            <el-upload
              class="upload-demo"
              action="http://localhost"
              :file-list=want.annex
              list-type="text">
            ...
            <br><br>
            <span>预期工时</span><el-input placeholder="数字" v-model="want.working" class="input2"></el-input>
            <span>{{hourOrDay}}</span>
            <span style="margin-left:5%">总薪资</span><el-input placeholder="数字" v-model="want.salary" class="input2"></el-input>
            <span>元</span>
...
export default {
  ...
  methods: {
    ...
    postWant () {
      console.log(this.want.annex)
      let flag = []
      for (let item in this.want) {
        flag.push(this.want[item].length)
      }
      if (flag.indexOf(0) <= -1) {
        this.$message.error('请填写缺少项')
      } else {
